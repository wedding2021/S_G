...
function scrollPartecipazione() {
    let id = 'partecipazione'
    if ($(window).width() < 767) {
...
function downloadPDF(pdf) {
    ...
    const fileName = "Invito.pdf";
    downloadLink.href = linkSource;
    downloadLink.download = fileName;
    downloadLink.click();
}
function download() {
    $.get("https://script.google.com/macros/s/AKfycbxXOmS-D4j8QzdGbluIH8erhW3IEggOIbiuseWBC-vnTWh7IWBOzBoDG1FbUqE7yZFtpQ/exec").done((data) => {
        dataJSON = $.parseJSON(data);
        downloadPDF(dataJSON['b64']);
    })
}
...
$(document).ready(function () {
    ...
    var r = createCalendar({
        options: { class: "", id: "" },
        data: {
            title: decodeURIComponent(escape(window.atob("TWF0cmltb25pbw=="))),
            start: new Date(decodeURIComponent(escape(window.atob("T2N0IDIyLCAyMDIxIDExOjAw")))),
            end: new Date(decodeURIComponent(escape(window.atob("T2N0IDIyLCAyMDIxIDIwOjAw")))),
            address: decodeURIComponent(escape(window.atob("UGF0ZXJuw7IsIENoaWVzYSBTYW50YSBCYXJiYXJh"))),
            description: decodeURIComponent(escape(window.atob("TWF0cmltb25pbyBkaSBTYWx2YXRvcmUgZSBHaXVzZXBwaW5h")))
        }
    });
    $("#add-to-cal").html(r),
        $("#rsvp-form").on("submit", function (e) {
            ...
            $("#alert-wrapper").html(
                alert_markup("info", "<strong>Attendi qualche secondo!</strong>")
            ),
        ...
        "2ac7f43695eb0479d5846bb38eec59cc" !== MD5($("#invite_code").val())
          ...
          : */ $.post(
                "https://script.google.com/macros/s/AKfycbw_ECUZosib8LqTSioiTGsfBaFCJxhybm2y2zO9reUmB0BHw0TEsfO8Ss9jivcXgB-Rmw/exec",
                t
            )
                    .done(function (e) {
                        "error" === e.result
                            ...
                            : ($("#alert-wrapper").html(""),
                                $("#rsvp-modal").modal("show"), download());
                    })
                    .fail(function (e) {
                        console.log(e),
                            $("#alert-wrapper").html(
                                alert_markup(
                                    "danger",
                                    "<strong>Attenzione:</strong> si è verificato un errore con il server! "
                                )
                            );
                    });
        });
